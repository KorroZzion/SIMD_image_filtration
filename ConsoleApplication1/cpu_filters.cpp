        ...
        for (int j = 0; j < input.cols; ++j) {
            ...
            for (int ki = -halfSize; ki <= halfSize; ++ki) {
                for (int kj = -halfSize; kj <= halfSize; ++kj) {
                    ...
                    if (x < 0) x = 0; // Крайний верх
                    if (x >= input.rows) x = input.rows - 1; // Крайний низ
                    if (y < 0) y = 0; // Крайний левый
                    if (y >= input.cols) y = input.cols - 1; // Крайний правый
...
void adjustSaturationSIMD(const cv::Mat& input, cv::Mat& output, float saturationScale) {
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j <= width - 8; j += 8) { // Обрабатываем по 8 пикселей за раз
            // Загружаем 8 пикселей
            __m256i pixels = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&input.at<cv::Vec3b>(i, j)));

            // Извлекаем отдельные каналы B, G, R
            ...
            // Преобразуем в формат float для вычислений
            ...
            __m256 new_blue = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(blue_f, intensity), _mm256_set1_ps(saturationScale)), intensity);
            __m256 new_green = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(green_f, intensity), _mm256_set1_ps(saturationScale)), intensity);
            __m256 new_red = _mm256_add_ps(_mm256_mul_ps(_mm256_sub_ps(red_f, intensity), _mm256_set1_ps(saturationScale)), intensity);
